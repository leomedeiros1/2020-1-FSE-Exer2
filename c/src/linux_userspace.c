...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    while (1)
    {
        double acc_temp = 0, acc_press = 0, acc_hum = 0;
        ...
        if(file){
            ...
            // CSV Header
            fprintf(file, "Temperature (oC), Pressure (hPa), Humidity (%%), Data\n");
        }
        ...
        if(file){
            ...
            fprintf(file, "%0.2lf, %0.2lf, %0.2lf, %s\n", acc_temp/10, acc_press/10, acc_hum/10, asctime (timeinfo));
            printf("Data saved in CSV\n");
        }
        else{
